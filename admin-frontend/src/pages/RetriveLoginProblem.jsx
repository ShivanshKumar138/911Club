...
const RetrieveLogin = () => {
  ...
  const fetchTickets = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://api.747lottery.fun  /retriveId');
      
      if (response.data.success) {
        // Transform API data to match the table structure
        const formattedTickets = response.data.retrivalTickets.map((ticket) => {
          // Format the date
          ...
          return {
            id: ticket._id,
            accountNumber: ticket.bankAccountNumber,
            ...
            image: ticket.depositReceiptImage.startsWith('http') 
              ? ticket.depositReceiptImage 
              : `https://api.747lottery.fun  ${ticket.depositReceiptImage}`
          };
        });
                              ...
                              <Box margin={2}>
                                <Paper className="p-4 bg-gray-50">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                      <Typography className="font-medium">Bank Account Number:</Typography>
                                      <Typography className="text-gray-700">
                                        {ticket.accountNumber.substring(0, 4)}...{ticket.accountNumber.substring(ticket.accountNumber.length - 4)}
                                      </Typography>
                                    </div>
                                    <div>
                                      <Typography className="font-medium">IFSC Code:</Typography>
                                      <Typography className="text-gray-700">{ticket.ifscCode}</Typography>
                                    </div>
                                    <div>
                                      <Typography className="font-medium">Email:</Typography>
                                      <Typography className="text-gray-700">{ticket.email}</Typography>
                                    </div>
                                    <div>
