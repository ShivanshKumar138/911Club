...
const SecondDepositBonus = () => {
  ...
  const fetchDepositBonuses = async () => {
    try {
      const response = await apiCall('/all-second-deposit-bonuses');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await apiCall(`/admin/update-second-deposit-bonus`, {
        method: 'PUT',
        body: JSON.stringify({
  ...
  const handleDelete = async (minimumDeposit) => {
    try {
      const response = await apiCall('/second-deposit-bonus', {
        method: 'DELETE',
        body: JSON.stringify({
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-blue-500">Second Deposit Bonus</h1>
...
export default SecondDepositBonus;