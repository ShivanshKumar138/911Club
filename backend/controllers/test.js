  ...
  const jobFunction = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        // console.log(
        //   `[${timerName}] Manual result:`,
        //   manualResult ? "Found" : "Not found"
        // );
        ...
        // console.log(
        //   `[${timerName}] Calculation bets count:`,
        //   calculationBets.length
        // );

        // Get all bets for updating results
        const allBets = await Bets.find({ periodId });
        // console.log(`[${timerName}] Total bets count:`, allBets.length);
        ...
        const isRandomWinning = gameWinningType
          ...
          : false;
        // console.log(
        //   `[${timerName}] Random winning:`,
        //   isRandomWinning ? "Enabled" : "Disabled"
        // );
        ...
        if (manualResult) {
          ...
          // console.log(`[${timerName}] Calculating outcome based on least bets`);
...
if (calculationBets.length === 0) {
  ...
  // Single player logic with 30% win chance
  const singleBet = calculationBets[0];
  const willWin = shouldWinWithProbability(0.3); // 30% chance to win
  ...
  if (willWin) {
    ...
    if (/^[0-9]$/.test(singleBet.selectedItem)) {
    ...
    } else if (singleBet.selectedItem === "big" || singleBet.selectedItem === "small") {
      // Size bet
      sizeOutcome = singleBet.selectedItem;
      // Find a random number that matches the size
      const possibleNumbers = Array.from({ length: 10 }, (_, i) => i)
        .filter(num => singleBet.selectedItem === "big" ? num > 4 : num <= 4);
      numberOutcome = possibleNumbers[Math.floor(Math.random() * possibleNumbers.length)].toString();
      colorOutcome = NUMBER_COLOR_MAP[numberOutcome];
    } else {
      // Color bet
      const possibleNumbers = Object.entries(NUMBER_COLOR_MAP)
        ...
        .map(([num]) => num);
      numberOutcome = possibleNumbers[Math.floor(Math.random() * possibleNumbers.length)];
      sizeOutcome = parseInt(numberOutcome) > 4 ? "big" : "small";
      colorOutcome = NUMBER_COLOR_MAP[numberOutcome];
    }
  } else {
    ...
    if (/^[0-9]$/.test(singleBet.selectedItem)) {
      ...
      const possibleNumbers = Array.from({ length: 10 }, (_, i) => i.toString())
        .filter(num => num !== singleBet.selectedItem);
      numberOutcome = possibleNumbers[Math.floor(Math.random() * possibleNumbers.length)];
    } else if (singleBet.selectedItem === "big" || singleBet.selectedItem === "small") {
      // For size bet, choose opposite size
      sizeOutcome = singleBet.selectedItem === "big" ? "small" : "big";
      numberOutcome = findNumberForSize(sizeOutcome);
    } else {
      // For color bet, choose different color
      const possibleColors = ["red", "green", "violet"].filter(c => c !== singleBet.selectedItem);
    ...
    colorOutcome = NUMBER_COLOR_MAP[numberOutcome];
            ...
            const totalColorBets = getTotalBetsInCategory(betSums.colorBetSums);
            ...
            // console.log(`[${timerName}] Category totals:`, categories);
...
// Update the bet processing section in the Promise.all block
await Promise.all(
  allBets.map(async (bet) => {
    ...
    if (["Restricted", "Admin", "Normal"].includes(bet.userType)) {
      ...
      if (bet.selectedItem === numberOutcome) {
        ...
        if (bet.selectedItem === "violet") {
          ...
          const multiplier = (numberOutcome === "0" || numberOutcome === "5") 
    ...
    // Log bet result
    // console.log(`[${timerName}] Bet processed for user ${bet.userId}:
    //   Selected: ${bet.selectedItem}
    //   Bet Amount: ${bet.totalBet}
    //   Win/Loss: ${winLoss}
    //   Status: ${status}`);
    ...
    await Bets.findByIdAndUpdate(bet._id, {
      ...
      result: numberOutcome,
        ...
        // console.log(`[${timerName}] Period ${periodId} completed:
        //   Total Bets: ${totalBetAmount}
        //   Total Wins: ${totalWinAmount}
        //   Total Tax: ${totalTax}
