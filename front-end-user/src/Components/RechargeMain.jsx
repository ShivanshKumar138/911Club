...
const PromotionMain = ({ children }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (paymentMode === "USDT") {
        ...
        console.log(
          "API call to /create-usdt-collection-order completed. Response:",
          response
        );
        ...
        if (response.data) {
          ...
          console.error(
            "Invalid response structure or missing payUrl",
            response.data
          );
        ...
        console.log(
          "API call to /create-btcash-order completed. Response:",
          response
        );
        ...
        console.log(
          "API call to /create-rupee-order completed. Response:",
          response
        );
  ...
  if (!isAllPaymentMethodOn) {
    // Only show UPIxPAYTM if isAllPaymentMethodOn is false
    keysToDisplay = ["UPIxPAYTM"];
  } else if (accountType === "Restricted") {
    // Only show UPI x QR for restricted accounts
    keysToDisplay = ["UPI x QR"];
  } else {
    // Show all payment methods except UPI x QR for normal accounts
    keysToDisplay = Object.keys(paymentModes).filter(
      (key) => key !== "UPI x QR"
    );
  }
                  ...
                  >
                    <Box
                      sx={{
                        ...
                        height: "35px",
                        clipPath:
                          "polygon(0 0, 100% 0, 100% 40%, 85% 100%, 15% 100%, 0 40%)",
                        display: "flex",
                        flexDirection: "column",
                        alignItems: "center",
                        justifyContent: "center",
